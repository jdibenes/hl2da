...
void Uahat_viewer::TickComponent(float DeltaTime, ELevelTick TickType, FActorComponentTickFunction* ThisTickFunction)
{
	...
	tex[0]->UpdateTextureRegions(0, 1, region.get(), 512 * 2, 2, (uint8_t*)fb->Buffer(0), [fb](uint8*, FUpdateTextureRegion2D const*) { fb->Destroy(); });
	tex[1]->UpdateTextureRegions(0, 1, region.get(), 512 * 2, 2, (uint8_t*)fb->Buffer(1), [fb](uint8*, FUpdateTextureRegion2D const*) { fb->Destroy(); });
}
