...
// OK
int SpatialInput_GetHandPose(SpatialCoordinateSystem const& world, PerceptionTimestamp const& ts, JointPose* left_poses, JointPose* right_poses)
{
    ...
    ok = pose.TryGetJoints(world, g_joints, { target, HAND_JOINTS });
    if (!ok) { continue; }
