...
// OK
static bool ExtendedVideo_FindVideoFormat(MediaFrameSource const& videoSource, uint32_t width, uint32_t height, uint32_t frameRate, winrt::hstring const& subtype, MediaFrameFormat& selectedFormat)
{
    ...
    if (format.Subtype()                                          != subtype)   { continue; }
...
// OK
bool ExtendedVideo_SetFormat(uint16_t width, uint16_t height, uint8_t framerate, winrt::hstring const& subtype)
{
